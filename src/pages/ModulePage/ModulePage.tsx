...
export default function ModulePage() {
    ...
    async function GetCourse() {
        ...
        setisLoading(true)
        const response = await axios.post('http://course-generation-server-production.up.railway.app/api/generateLesson',
    ...
    async function getpractice() {
        ...
        if (storecourse.length > 0 && activecourse!.modules.find(v => v.id === activemoduleid)!.lessons.find(v => v.id === activelessonid)!.practice.length == 0) {
            const response = await axios.post('http://course-generation-server-production.up.railway.app/api/generatePractice',
